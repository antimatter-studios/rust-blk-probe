//! One check gates a pull request, and it stands for every job (#18).
//!
//! Branch protection names checks, and a check is a job name. This repository
//! named three: `fmt`, `test / ubuntu-latest` and `test / macos-latest` — and
//! two of those are legs of ONE matrix, whose names come from its `os:` list.
//! Adding the `ubuntu-24.04-arm` leg that #15 needs would have produced a
//! check reporting on every pull request and gating nothing, because nobody
//! would have edited the settings page in the same breath. The opposite
//! spelling is worse: a required check no job produces reads as permanently
//! pending, and with `enforce_admins` on nothing merges and there is no
//! failure to point at.
//!
//! So protection names one job, `ci-ok`, which `needs:` every other job in
//! `ci.yml` and fails unless each concluded success. `needs:` names the matrix
//! job rather than its legs, so a leg added to `os:` is waited on the moment
//! it exists.
//!
//! An aggregate nobody checks is a worse gate than the pinned list it
//! replaced: it can quietly stop watching a job and still report green. This
//! is what keeps it honest — every job in `ci.yml` has to be in its `needs`,
//! it has to carry `if: always()`, and `.github-guard` has to require it and
//! nothing else.
